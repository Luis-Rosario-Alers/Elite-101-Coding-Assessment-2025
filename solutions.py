...
def list_all_free_tables(time_slot: int, table_data: list) -> list:
    ...
    for occupied_state in range(1,
                                len(table_data)[time_slot]):  # here we check make a for loop that skips the first index
...
def list_all_free_tables_and_combines_tables_friendly(time_slot: int, table_data: list, party_size: int) -> list:
    """
    ...
    for occupied_state in range(1, len(table_data[time_slot])):  # loop over the occupied state in given time slot
        ...
        if is_open and get_table_capacity(current_table) >= party_size:  # if table is open and has a capacity larger or equal to party size, we append it to the free tables list.
            ...
            if prev_is_open and next_is_open and (get_table_capacity(prev_table) + get_table_capacity(next_table)) >= party_size: # if the capacity of both adjacent tables are bigger than or equal to the party size
    ...
    for tables in free: # looping through all the free tables
        if isinstance(tables, list): # if the element is a list, it means it is a combo of 2 tables
            table1_size = tables[0][tables[0].find('(') + 1:tables[0].find(')')]
            table2_size = tables[1][tables[1].find('(') + 1:tables[1].find(')')]
            print(f"combining table {table1_size} + table {table2_size} will be enough to fit your party size.") # we then print to the user both tables combined will accommodate their party size
        else: # else we just print the table and say its free
            print(f"table {tables[tables.find('(') + 1:tables.find(')')]} is free and big enough for your party.")
